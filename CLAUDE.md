...
3. Bidi trap: private-use characters count as left-to-right, and spaces between two LTR words become LTR, so a verse would read from the left. Each word is therefore followed by an invisible RLM (U+200F) before the space; the font carries U+200F as an empty glyph.
...
5. Changing only the data or app code needs just `palm-package` + `palm-install`; changing the font needs the font install + Luna restart.
...
Last thing done, **not yet confirmed by the user on the device**: the RLM word-order fix (see below), the added space between Arabic and English, and the new "Verse numbers" menu (Arabic-style vs regular digits). Next session: ask the user how those look, then fix anything off.
